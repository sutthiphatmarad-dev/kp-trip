...
- วัดพระแก้ว กำแพงเพชร (⭐ 4.6) | พิกัด: 16.4935, 99.5160
- กำแพงเมืองเก่า กำแพงเพชร (⭐ 4.4) | พิกัด: 16.4910, 99.5185
- วัดพระนอน กำแพงเพชร (⭐ 4.5) | พิกัด: 16.4980, 99.5145
- บ่อสามแสน (⭐ 4.2) | พิกัด: 16.5010, 99.5100
- วัดช้างเผือก (⭐ 4.3) | พิกัด: 16.4988, 99.5132
- เจดีย์ทอง วัดเจดีย์กลางทุ่ง นครชุม (⭐ 4.4) | พิกัด: 16.4805, 99.5060
"""
...
- อุทยานแห่งชาติแม่วงก์ (⭐ 4.6) | พิกัด: 16.0900, 99.1100
- น้ำตกคลองลานน้อย (⭐ 4.4) | พิกัด: 16.1450, 99.2700
- อ่างเก็บน้ำคลองน้ำไหล (⭐ 4.2) | พิกัด: 16.1700, 99.2600
- เขาหลวง คลองลาน (⭐ 4.3) | พิกัด: 16.1550, 99.2400
"""
...
- คาเฟ่ อะมาโด้ กำแพงเพชร (⭐ 4.4) | พิกัด: 16.4830, 99.5232
- ร้านกาแฟริมปิง (⭐ 4.3) | พิกัด: 16.4845, 99.5215
- ครัวชากังราว (⭐ 4.3) | พิกัด: 16.4790, 99.5240
- ก๋วยเตี๋ยวเรือ นครชุม (⭐ 4.5) | พิกัด: 16.4835, 99.5095
- ร้านลาบเป็ด กำแพงเพชร (⭐ 4.2) | พิกัด: 16.4872, 99.5258
- ขนมจีนชากังราว (⭐ 4.4) | พิกัด: 16.4810, 99.5188
"""
...
- เพชร โฮเต็ล กำแพงเพชร (⭐ 4.2) | พิกัด: 16.4760, 99.5260
...
def filter_places_near_route(places_text, start_coords, end_coords, max_km=40, limit=18):
    """
    กรองสถานที่ตามตำแหน่ง (แบบผสม):
    - ถ้าจุดเริ่ม≈จุดจบ (เที่ยวแถวเดียว, ห่างกัน <10 กม.) -> ใช้ "รัศมีจากจุดกึ่งกลาง"
    - ถ้าข้ามเมือง (ไกลกว่านั้น) -> ใช้ "ระยะจากแนวเส้นทาง เริ่ม->จบ"
    เรียงจากใกล้สุดก่อน คืน "" ถ้าไม่มีสักที่ (ให้ผู้เรียกใช้ fallback)
    """
    try:
    ...
    # ระยะจุดเริ่ม -> จุดจบ
    trip_len = point_to_segment_km(ealat, ealon, salat, salon, salat, salon)
    use_radius = trip_len < 10.0  # ใกล้กันมาก = เที่ยววนแถวเดียว
    cx, cy = (salat + ealat) / 2.0, (salon + ealon) / 2.0  # จุดกึ่งกลาง
    ...
    for line in (places_text or "").strip().split('\n'):
        ...
        if use_radius:
            d = point_to_segment_km(plat, plon, cx, cy, cx, cy)        # รัศมีจากจุดกึ่งกลาง
        else:
            d = point_to_segment_km(plat, plon, salat, salon, ealat, ealon)  # ระยะจากเส้นทาง
        if d <= max_km:
            kept.append((d, line))
...
def fetch_live_places(keyword="สถานที่ท่องเที่ยว"):
    ...
    try:
        ...
        for place in data.get('places', []):
            ...
            if total_ratings >= 0:  # รวมทุกที่ (เดิมตัดที่รีวิว <= 3 ออก ทำให้ได้น้อย)
    ...
    try:
        ...
        for i in range(num_days):
            ...
            day_attr = (filter_places_near_route(db_attractions_main, s_coords, e_coords, max_km=40, limit=18)
                        or _cap_lines(db_attractions_main, 14))
            day_nature = (filter_places_near_route(db_attractions_outside, s_coords, e_coords, max_km=60, limit=10)
                          or _cap_lines(db_attractions_outside, 8))
            day_food = (filter_places_near_route(db_food, s_coords, e_coords, max_km=40, limit=14)
                        or _cap_lines(db_food, 10))
